...
import { generateMagicConcept } from '../services/aiService';
...
const MagicConcierge: React.FC = () => {
  ...
  const handleMagic = async () => {
    ...
    try {
      const response = await generateMagicConcept(prompt);
      setResult(response);
    } catch (e) {
      console.error(e);
      // This should rarely happen as generateMagicConcept has its own fallback
      setResult({
        title: "Erreur",
        tagline: "Une erreur inattendue s'est produite.",
        concept: "Le processus d'invocation a échoué de manière inattendue. Veuillez réessayer."
      });
    } finally {
      setLoading(false);
    }
